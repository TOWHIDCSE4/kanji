...
class NavBar extends Component {
  ...
  goBack() {
    ...
    if (props.location.pathname.includes('grade') || props.location.pathname.includes('chapter')) {
      props.history.push('/tohi-kanji/', props.state);
    } else {
  ...
  render() {
    ...
    return (
      <nav data-testid="navbar" className={navbar.nav}>
        <Link to="/tohi-kanji/" className={navbar.logo}>
          <span className={navbar.logo__kanji}>青</span>
          KANJI
        </Link>
        { props.location.pathname !== '/tohi-kanji/' && <i role="button" tabIndex={-1} aria-label="Back to last page" onClick={this.clickHandler} onKeyDown={e => this.handleKeyDown(e)} className={navIcon} />}
        <SearchBar
          key={state.text}
